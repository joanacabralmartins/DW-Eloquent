...
@section('conteudo')
    <form action="{{ route('alunos.store') }}" method="POST">
        ...
        <div class="d-flex gap-3 mb-4">
            <div>
                <h5><span class="badge bg-secondary">Curso</span></h5>
            </div>
            <div class="flex-grow-1">
                <select class="form-select form-select-sm" name="curso" id="id" required>
                        <option value=""></option>
                            @foreach ($cursos as $curso)
                                <option value="{{ $curso->id}}" {{ $curso->id == old('curso') ? 'selected' : '' }}>{{ $curso->nome }}</option>
                            @endforeach
                </select>
            </div>
        </div>
